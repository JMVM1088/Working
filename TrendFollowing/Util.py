...
def insert_prices_sql(conn, df,table_name ):
    ...
    print(f"  Inserted {df.shape[0]} rows for {df['Ticker'].iloc[0]} (skipped duplicates)")
############################################################################################################################################
def insert_IntraDayPrices_sql(conn, df,table_name ):
    cursor = conn.cursor()
    for _, row in df.iterrows():
        # Insert new row
        adj_close_val = max(0.00, min(row['Adj Close'], 1000000.00))
...
def get_business_days_decrement(start_date, end_date, decrement_days=10):
    ...
    return date_pairs
############################################################################################################################################
def getDailyPrice(connStr,stockType, destTable, lookbackDays,PriceType ):    
    # Get the current date and time
    Start_datetime = datetime.now()
    conn = pyodbc.connect(connStr)
    ##stock_list = list(Util.get_data_from_sql(connection_string, f"SELECT symbol FROM [Stock].[dbo].[StockList_US]"))
    stock_list = list(get_data_from_sql(connStr, f"exec stock..sp_GetStockList '{stockType}'"))

    for ticker in stock_list:
                df = get_prices(ticker[0], lookbackDays)
                if df is not None and not df.empty:
                    if PriceType == 'I':  ## IntraDay  
                        insert_IntraDayPrices_sql(conn, df, f'{destTable}')
                    else:  ## Historical 
                        insert_prices_sql(conn, df, f'{destTable}')
