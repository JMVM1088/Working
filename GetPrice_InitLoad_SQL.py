...
stock_list = list(Util.get_data_from_sql(connection_string, "exec stock..sp_GetStockList_ETF"))

for ticker in stock_list:
    for begin_date, end_date in reversed(result_pairs):
            df = Util.get_prices_range(ticker[0], begin_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'))
            if df is not None and not df.empty:
                Util.insert_prices_sql(conn, df, 'Historical_ETF')
