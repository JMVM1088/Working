"""
🚀 DUAL-HORIZON DQN BACKTEST - LONG ONLY MODEL (2025 DATA)
✅ UPDATED: Uses dqn_spy_improved model
✅ Runs for all of 2025
✅ Uses softmax for normalized confidence (0-1 range)
✅ All signals will now include LONG positions
"""
...
print("🎯 DUAL-HORIZON DQN BACKTEST - dqn_spy_improved (2025)\n")
...
# ✅ UPDATED: Run for entire 2025
BACKTEST_START = "2025-12-20"
BACKTEST_END = "2025-12-19"
MODEL_PATH = "dqn_spy_improved"  # ✅ UPDATED: Path to improved model directory
...
print(f"   Model:             dqn_spy_improved")
...
def run_backtest(model_path: str, df: pd.DataFrame, symbol: str = "SPY"):
    ...
    try:
        ...
        print(f"✅ Loaded DQN model: dqn_spy_improved\n")
    except Exception as e:
    ...
    print(f"   Using SOFTMAX confidence calculation\n")
    ...
    for i in range(len(df)):
        try:
            ...
            if (i + 1) % 63 == 0:
                long_pct = 100 * len([p for p in predictions if p['signal'] == 'LONG']) / len(predictions)
                print(f"  Day {i+1:4d}: Equity=${equity:.4f} | Signal={signal:4s} | Pos={position:.2f} | Conf={confidence:.2f} | LONG%={long_pct:.1f}%")
    ...
    print(f"   FLAT signals:    {flat_days} ({flat_days/len(df_preds)*100:.1f}%)")
    print(f"   LONG signals:    {long_days} ({long_days/len(df_preds)*100:.1f}%)")
...
def analyze_backtest(df_preds: pd.DataFrame) -> dict:
    ...
    print(f"{'='*70}")
    print(f"📊 BACKTEST ANALYSIS - dqn_spy_improved (2025)")
    print(f"{'='*70}\n")
    ...
    print(f"   Max Position:       {df_preds['position'].max():.3f}")
...
if __name__ == "__main__":
    ...
    try:
        print(f"\n{'='*70}")
        print(f"🚀 DQN BACKTEST - dqn_spy_improved (2025 DATA)")
        print(f"{'='*70}\n")
        
        print(f"STEP 1: Load 2025 data")
        print(f"-" * 70)
        df_raw = load_data(SQL_CONN, SYMBOL, BACKTEST_START, BACKTEST_END)
        
        print(f"STEP 2: Compute features")
        print(f"-" * 70)
        df = compute_advanced_features(df_raw)
        
        print(f"STEP 3: Run backtest")
        print(f"-" * 70)
        df_preds = run_backtest(MODEL_PATH, df, SYMBOL)
        ...
        print(f"-" * 70)
        run_id = f"backtest_{SYMBOL}_improved_2025_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        save_to_sql(SQL_CONN, df_preds, run_id)
        
        print(f"\n{'='*70}")
        print(f"✅ BACKTEST COMPLETE - dqn_spy_improved (2025 DATA)")
        print(f"{'='*70}\n")
        ...
        print(f"   Model: dqn_spy_improved")
        print(f"   Trained on: 2015-2024 data")
        print(f"   Backtested on: 2025 data\n")
