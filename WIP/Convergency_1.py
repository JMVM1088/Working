...
def get_ohlcv_from_sqlite(
    db_file: str,
    ticker: str,
    table_name: str = 'Historical_SP500'
) -> pd.DataFrame:
...
def get_symbol_list_from_sqlite(
    db_file: str,
    table_name: str = 'HistoricalPrices'
) -> List[str]:
    """
    Return distinct list of tickers from the SQLite table.
    """
    db = db_file or getattr(Util, 'DB_FILE', None)
...
def scan_patterns_for_all_symbols(
    db_file: str,
    table_name: str,
    anchor_date: str,
    lookback_days: int = 60,
    pivot_window: int = 2,
    algo_type: str = AlgorithmType.ENHANCED,
    output_csv: str = "patterns_found.csv",
) -> pd.DataFrame:
    """
    For each distinct symbol in the SQLite table, run pattern analysis at anchor_date.
    If a converging pattern is found, collect summary info and save to CSV.
    """
    symbols = get_symbol_list_from_sqlite(db_file, table_name)
    results: List[Dict] = []
    ...
    for ticker in symbols:
        ...
        if res.get('found'):
            # compact summary row for CSV
            results.append({
                'Ticker': ticker,
                'AnchorDate': anchor_date,
                'Maturity': res.get('maturity'),
                'WidthStart': res.get('widthStart'),
                'WidthEnd': res.get('widthEnd'),
                'BreakoutSameBar': res.get('breakout'),
                'BreakoutTypeSameBar': res.get('breakoutType'),
                'BreakoutVolumeFactor': res.get('breakoutVolumeFactor'),
                'ResSlope': res['resistanceLine']['slope'] if res.get('resistanceLine') else None,
                'SupSlope': res['supportLine']['slope'] if res.get('supportLine') else None,
                'ResAtAnchor': res.get('resistanceAtAnchor'),
                'SupAtAnchor': res.get('supportAtAnchor'),
                'NumPivotsHigh': len(res.get('pivotsHigh', [])),
                'NumPivotsLow': len(res.get('pivotsLow', [])),
            })
    ...
    if results:
        ...
        df_out = pd.DataFrame(columns=[
            'Ticker','AnchorDate','Maturity','WidthStart','WidthEnd',
            'BreakoutSameBar','BreakoutTypeSameBar','BreakoutVolumeFactor',
            'ResSlope','SupSlope','ResAtAnchor','SupAtAnchor',
            'NumPivotsHigh','NumPivotsLow'
        ])
...
if __name__ == '__main__':
    ...
    anchor_date = '2025-11-24'  # pre-breakout date you want to scan
    lookback_days = 60
    pivot_window = 2
    algo_type = AlgorithmType.ENHANCED
    output_csv = rf"C:\Users\jv2mk\OneDrive\Stock\working\Stock\Working\Result\{anchor_date}_pattern_scan_results.csv"

    df_patterns = scan_patterns_for_all_symbols(
        db_file=db_file,
    ...
    print(f"Patterns found: {len(df_patterns)}")
