"""
🚀 DUAL-HORIZON DQN BACKTEST WITH 12 ADVANCED FEATURES
✅ 5-year backtest (2020-2025) ✅ 20 features ✅ Full analysis
"""
...
print("🎯 DUAL-HORIZON DQN ADVANCED BACKTEST")
...
def run_backtest(model_path: str, df: pd.DataFrame, symbol: str = "SPY"):
    """
    Run 5-year backtest on advanced model.
    ...
    # Load normalization stats for 20 features
    feat_mean = np.load("feat_mean_advanced.npy")
    feat_std = np.load("feat_std_advanced.npy")
...
def analyze_backtest(df_preds: pd.DataFrame) -> dict:
    ...
    print(f"\n📈 ADVANCED FEATURES CORRELATION:")
    
    # Feature analysis
    print(f"\n   Momentum (RSI, MACD):")
    corr_rsi_pnl = df_preds['rsi_14'].corr(df_preds['daily_pnl'])
    corr_macd_pnl = df_preds['macd_hist'].corr(df_preds['daily_pnl'])
    print(f"      RSI-PnL correlation: {corr_rsi_pnl:.3f}")
    print(f"      MACD-PnL correlation: {corr_macd_pnl:.3f}")
    
    print(f"\n   Volatility (ATR, ADX):")
    corr_atr_pnl = df_preds['atr_14'].corr(df_preds['daily_pnl'])
    corr_adx_pnl = df_preds['adx_14'].corr(df_preds['daily_pnl'])
    print(f"      ATR-PnL correlation: {corr_atr_pnl:.3f}")
    print(f"      ADX-PnL correlation: {corr_adx_pnl:.3f}")
    
    print(f"\n   Price Action (BB, Range):")
    corr_bb_pnl = df_preds['bb_position'].corr(df_preds['daily_pnl'])
    corr_range_pnl = df_preds['range_norm'].corr(df_preds['daily_pnl'])
    print(f"      BB-PnL correlation: {corr_bb_pnl:.3f}")
    print(f"      Range-PnL correlation: {corr_range_pnl:.3f}")
    
    print(f"\n   Volume (OBV):")
    corr_obv_pnl = df_preds['obv_ratio'].corr(df_preds['daily_pnl'])
    print(f"      OBV-PnL correlation: {corr_obv_pnl:.3f}")
...
if __name__ == "__main__":
    ...
    try:
        print(f"\n{'='*60}")
        print(f"🚀 DUAL-HORIZON BACKTEST - ADVANCED MODEL")
        print(f"{'='*60}\n")
        ...
        # ===== STEP 3: Run Backtest =====
        print(f"\nSTEP 3: Run backtest with advanced model")
        print(f"-" * 60)
        ...
        print(f"\nSTEP 5: Save to SQL")
        print(f"-" * 60)
        run_id = f"backtest_{SYMBOL}_advanced_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
