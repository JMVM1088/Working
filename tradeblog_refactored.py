...
Date: 2026-01-27
Purpose: Professional multi-symbol, multi-strategy options tracking with flexible closure

SUPPORTED SYMBOLS: SPY, QQQ, IWM, individual stocks (TSLA, AAPL, etc.)
SUPPORTED STRATEGIES: Iron Condor, Call/Put Spreads, Single Legs, Straddle, Strangle
EARLY CLOSURE: Manually mark batches as closed before expiration

DATABASE: SQL Server (enhanced schema with Trade_Legs and Trade_StrategyConfigs tables)
"""
...
# Strategy configurations: leg_count, [leg1_type, leg1_side, leg2_type, leg2_side, ...]
STRATEGY_CONFIGS = {
...
class GreeksEngine:
    ...
    def __init__(self, symbol: str = "SPY", risk_free_rate: float = 0.05):
        """
        Args:
            symbol: Underlying ticker (SPY, QQQ, IWM, TSLA, etc.)
    ...
    def get_legs_snapshot(self, legs: List[OptionLeg]) -> Dict:
        ...
        # Validate all legs have same expiry and symbol
        ...
        for leg in legs:
            ...
            if leg.symbol != symbol:
                raise ValueError("All legs must have same symbol")
        ...
        return {
            ...
            "spot": self.get_option_greeks(
                expiry, legs[0].strike, legs[0].option_type.value.lower()
            )["spot"],
            "expiry": expiry,
...
class TradeBlogManager:
    ...
    def insert_batch(self, batch_data: Dict) -> int:
        ...
        Args:
            batch_data: Dict with keys:
                - batch_name: str
                - symbol: str (SPY, QQQ, IWM, TSLA, etc.)
                ...
                - entry_iv_rank: float (optional)
                - entry_vix: float (optional)
        """
        # Validate strategy
        ...
        params = (
            ...
            batch_data.get("symbol", "SPY").upper(),
            strategy.value,
        ...
        print(f"✅ Batch inserted: {batch_data['batch_name']} | {batch_data.get('symbol', 'SPY')} | {strategy.value} (ID: {batch_id})")
        return batch_id
    ...
    def get_batch_Old(self, batch_id: int) -> Dict:
    ...
    def get_batch(self, batch_id: int) -> Dict:
        """Retrieve batch details"""
        query = """
            SELECT BatchID, BatchName, Symbol, StrategyType, EntryDate, ExpirationDate, 
                IsClosed, CreditCollected, EntryPrice
        ...
        r = rows[0]
        return {
        "batch_id": r[0],           # ✅ Index 0
        "batch_name": r[1],         # ✅ Index 1
        "symbol": r[2],             # ✅ Index 2
        "strategy_type": r[3],      # ✅ Index 3
        "entry_date": r[4],         # ✅ Index 4
        "expiration_date": r[5],    # ✅ Index 5 - NOW a datetime!
        "isclosed": r[6],           # ✅ Index 6
        "credit_collected": r[7],   # ✅ Index 7
        "entry_price": r[8],        # ✅ Index 8
    }
    ...
    def get_batch_legs(self, batch_id: int) -> List[OptionLeg]:
        ...
        query = """
            SELECT LegID, LegNumber, OptionType, SideType, Strike, EntryPrice
            FROM [dbo].[Trade_Legs]
            WHERE BatchID = ?
            ORDER BY LegNumber
        """
        ...
        batch = self.get_batch(batch_id)
        symbol = batch.get("symbol", "SPY")
        expiry = batch.get("expiration_date").strftime("%Y-%m-%d") if batch.get("expiration_date") else None
        ...
        for batch in batches:
            ...
            try:
                ...
                snapshot_data = {
                    ...
                    "iv_rank": 55.0,  # Would be fetched from live source
                    "vix": 18.5,  # Would be fetched from live source
                    ...
                    "call_delta": 0.0,  # For multi-leg, already included in composite
                    "call_gamma": 0.0,
                    "call_vega": 0.0,
                    "call_theta": 0.0,
                    "current_pnl": 35.0,  # Would calculate from position
                    "probability_of_profit": 65.0,  # Would calculate
                    "expected_value": 42.5,  # Would calculate
...
if __name__ == "__main__":
    ...
    # # ====== EXAMPLE 3: IWM Call Spread (Bull Call) ======
    # print("\n[EXAMPLE 3] Creating IWM Bull Call Spread Batch")
    # print("-" * 80)

    # iwm_batch_data = {
    #     "batch_name": "IWM_BCS_Week1",
    #     "symbol": "IWM",
    #     "strategy_type": StrategyType.BULL_CALL_SPREAD,
    #     "entry_date": datetime.now().date(),
    #     "expiration_date": (datetime.now() + timedelta(days=21)).date(),
    #     "entry_price": 205.50,
    #     "credit_collected": 50.0,
    #     "number_of_spreads": 10,
    #     "entry_iv_rank": 45.0,
    #     "entry_vix": 14.0,
    # }

    # iwm_batch_id = manager.insert_batch(iwm_batch_data)
    ...
    # manager.insert_legs(iwm_batch_id, iwm_legs)
    ...
    print(f"Batch Status: Closed={batch['isclosed']}")


    print("\n" + "=" * 80)
    print("✅ Multi-Symbol, Multi-Strategy System Operational")
    print("=" * 80 + "\n")
