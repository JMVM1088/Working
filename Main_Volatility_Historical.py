...
for symbol in symbols:
    ...
    df = get_data(engine, "AI_stock_Prices", "[Time]", symbol[0], "2020-01-01", "2025-12-31")
    df = df.reset_index()                     # 🔑 make Date explicit
    df.rename(columns={"Date": "ReportDate"}, inplace=True)

    vol = yang_zhang(df, WINDOW_SIZE)

    # 🔑 DATE-SAFE construction
    tmp = pd.DataFrame({
    ...
    vol_df = pd.concat(valid_results).dropna()

    # 🔑 ensure datetime
    vol_df["ReportDate"] = pd.to_datetime(vol_df["ReportDate"])
    ...
    vol_df.to_sql(
        "AI_Stock_Volatility",
        engine,
        if_exists="append",   # ⚠️ never replace schema
        index=False
    )
    results = []  # reset for next symbol
    vol_df = pd.DataFrame()  # reset for next symbol
    valid_results = []  # reset for next symbol
