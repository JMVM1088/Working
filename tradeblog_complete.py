...
class TradeBlogManager:
    ...
    def insert_LegSnapshot(self, batch_id: int, runType, snapshot_data):
        """Insert leg snapshot for multi-strategy batch"""
        #snapshot_time = snapshot_data.get("snapshot_time", datetime.now())
        for leg in snapshot_data:
            # leg_snapshot_data = {
            #     "leg_id": leg.leg_id,
            #     "mid_price": leg.greeks["mid_price"],
            #     "iv": leg.greeks["iv"],
            ...
            # }
           # self._insert_single_leg_snapshot(batch_id, snapshot_time, leg_snapshot_data)

            leg_id = leg.leg_id
            if not leg_id:
                raise ValueError("leg_id is required in snapshot_data")
        
            query = """
                INSERT INTO [dbo].[Trade_LegSnapshots] (
                    LegID,
                    Price, IV,
                    Delta, Gamma, Vega, Theta, SnapshotType
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """
        
            params = (
                leg.leg_id,               
                leg.greeks["mid_price"],
                leg.greeks["iv"],
                leg.greeks["delta"],
                leg.greeks["gamma"],
                leg.greeks["vega"],
            ...
            leg_snapshot_id = self.db.execute_insert(query, params)
            print(f"✅ Leg Snapshot {leg_snapshot_id} for leg {leg_id} in batch {batch_id}")
    ...
    def capture_all_intraday(self) -> Dict:
        ...
        for batch in batches:
            ...
            try:
                ...
                # Snapshot Legs
                insert_LegSnapshot = self.manager.insert_LegSnapshot(batch_id,"IntraDay",legs)
                # Build snapshot data
                snapshot_data = {
    ...
    def capture_all_eod(self) -> Dict:
        ...
        for batch in batches:
            ...
            try:
                ...
                position_snap = greeks_engine.get_legs_snapshot(legs)
                 # Snapshot Legs
                insert_LegSnapshot = self.manager.insert_LegSnapshot(batch_id,"EOD",legs)
                snapshot_data = {
                    "spot_price": position_snap["spot"],
                    "iv_rank": 55.0,
                    "vix": 18.5,
